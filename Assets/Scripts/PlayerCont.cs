...
public class PlayerCont : MonoBehaviour
{
    ...
    [SerializeField] private float liftRadius;
    ...
    private GameObject boxToLift;
    private GameObject tempBox;
    private bool isLifting;
    private GameObject liftingBox;
    ...
    private void OnEnable()
    {
        ...
        thirdAction.performed += PlayerLift;
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (isLifting)
        {
            boxToLift.transform.position = liftPoint.position;
        }
    ...
    private void PlayerLift(InputAction.CallbackContext ctx)
    {
        if (isLifting) return;
        
        Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, liftRadius, LayerMask.GetMask("Liftable"));
        float previousLength = liftRadius;

        if (colliders.Length != 0)
        {
            foreach (Collider2D collider in colliders)
            {
                float colDist = Physics2D.Distance(collider, playerCol).distance;
                if (colDist < previousLength)
                {
                    tempBox = collider.gameObject;
                    previousLength = colDist;
                }
            }
